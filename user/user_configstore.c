...
void configSave() {
	...
	if (memcmp(&tmpConfig, &myConfig, sizeof(MyConfig))==0) return;

	//Okay, we need to save the config.
	myConfig.seq++;
	printf("MyConfig seq %d\n", (int)myConfig.seq);
