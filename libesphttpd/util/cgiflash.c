...
static char* checkBinHeader(void *buf) {
...
static char* checkEspfsHeader(void *buf) {
	if (memcmp(buf, "ESfs", 4)!=0) return "Bad ESPfs header";
	return NULL;
}
...
int cgiReadFlash(HttpdConnData *connData) {
	...
	write(connData->conn->sockfd, (uint8 *)(*pos), 1024);
	*pos+=1024;
